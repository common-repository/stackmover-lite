...
class Transfer implements PromisorInterface
{
     ...
     *   string $dest, WP2Aws\CommandInterface $command. The provided command will
    ...
    private function createDownloadPromise()
    {
        ...
        return (new WP2Aws\CommandPool($this->client, $commands, [
    ...
    private function createUploadPromise()
    {
        ...
        $files = \WP2Aws\map($this->getUploadsIterator(), function ($file) {
    ...
    private function getUploadsIterator()
    {
        if (is_string($this->source)) {
            return WP2Aws\filter(
                WP2Aws\recursive_dir_iterator($this->sourceMetadata['path']),
    ...
    private function getDownloadsIterator()
    {
        if (is_string($this->source)) {
            ...
            $files = $this->client
                ...
                ->search('Contents[].Key');
            $files = WP2Aws\map($files, function ($key) use ($listArgs) {
                return "s3://{$listArgs['Bucket']}/$key";
            });
            return WP2Aws\filter($files, function ($key) {
