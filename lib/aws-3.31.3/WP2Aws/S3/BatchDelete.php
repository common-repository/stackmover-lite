...
use WP2Aws\S3\Exception\DeleteMultipleObjectsException;
 ...
 * operation.
 *
 *     $s3 = new WP2Aws\S3\Client([
 *         'region' => 'us-west-2',
 *         'version' => 'latest'
 *     ]);
 *
 *     $listObjectsParams = ['Bucket' => 'foo', 'Prefix' => 'starts/with/'];
 *     $delete = WP2Aws\S3\BatchDelete::fromListObjects($s3, $listObjectsParams);
...
class BatchDelete implements PromisorInterface
{
     ...
     *
     * @param WP2AwsClientInterface $client            AWS Client to use.
     ...
     *
     * @param WP2AwsClientInterface $client  AWS Client to use to execute commands
    ...
    public static function fromIterator(
        ...
        $fn = function (BatchDelete $that) use ($iter) {
            return \WP2GuzzleHttp\Promise\coroutine(function () use ($that, $iter) {
                foreach ($iter as $obj) {
    ...
    /**
     * @param WP2AwsClientInterface $client    Client used to transfer the requests
