...
use WP2GuzzleHttp\Promise\EachPromise;
...
class CommandPool implements PromisorInterface
{
     ...
     *   The function is provided an WP2AwsException object, id of the iterator that
            ...
            foreach ($commands as $key => $command) {
                if (!($command instanceof CommandInterface)) {
                    throw new \InvalidArgumentException('Each value yielded by '
                        . 'the iterator must be an WP2Aws\CommandInterface.');
                }
                if ($before) {
                    $before($command, $key);
                }
     ...
     *
     * @return array
     * @see \WP2Aws\CommandPool::__construct for available configuration options.
     */
